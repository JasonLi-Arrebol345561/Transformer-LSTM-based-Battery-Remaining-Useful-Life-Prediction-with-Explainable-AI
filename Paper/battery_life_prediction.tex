...
\begin{abstract}
\noindent
快充策略对锂离子电池循环寿命的影响机制是电池管理系统优化的核心问题。本文基于MIT-Stanford 124颗A123 LFP 18650电池的快充老化数据集，构建\textbf{统计分析—机器学习预测—可解释性验证}的完整研究闭环，任务定义为\textbf{早期循环寿命预测}（用前100个循环预测总寿命）。在统计层面，采用Spearman秩相关、分层交互分析（Fisher r-to-z检验）与Benjamini-Hochberg FDR校正，量化充电策略参数（C1、Q1、C2）及各SOC区间倍率暴露对循环寿命的主效应与交互效应，发现Q1（阶段切换SOC）是显著的主效应与交互枢纽变量，C1$\times$Q1交互经检验显著（$|\Delta\rho|=0.824$，$p\approx5\times10^{-7}$）。在预测层面，采用四层渐进式特征体系与Transformer架构，在公平对比下（Ridge/RF与LSTM/Transformer输入同一套原始序列）5折交叉验证R$^2$达0.827，留出测试集（10次分层划分）R$^2$为$0.726\pm0.177$。在可解释层面，对Transformer本身采用Integrated Gradients、置换重要性与注意力rollout，并附随机初始化与时间轴打乱基线，验证内阻、充电时间与Q1的关键作用。三层分析共同揭示快充策略优化方向：低SOC区间集中高倍率充电、高SOC区间降倍率、保持两步倍率平滑过渡。
...
Python 3.12, PyTorch 2.14 (CPU), scikit-learn, scipy, h5py。训练在CPU上完成（124样本的小规模序列）。
...
\midrule
Ridge (展平序列900维)      & 178$\pm$31 & 236$\pm$50 & 0.577$\pm$0.097 \\
随机森林 (展平序列900维)    & 157$\pm$26 & 237$\pm$44 & 0.560$\pm$0.163 \\
LSTM             & 116$\pm$35 & 160$\pm$41 & 0.771$\pm$0.178 \\
\textbf{Transformer} & \textbf{110$\pm$24} & \textbf{146$\pm$32} & \textbf{0.827$\pm$0.092} \\
\bottomrule
\end{tabularx}
\end{table}

在公平对比（所有模型输入同一套前100循环原始序列）下，Ridge达到R$^2$=0.577——说明原先``Ridge R$^2=-0.09$''是让线性模型吃手工聚合特征所致的不公平对比，而非线性模型本身失败。Transformer仍最优（R$^2$=0.827），但优势幅度（相对Ridge约+0.25）比此前宣称的要温和。LSTM（0.771）介于随机森林与Transformer之间，方差较大。
...
\midrule
A (基础退化)     & 6  & 0.758$\pm$0.096 & ---    \\
B (+策略参数)    & 9  & 0.870$\pm$0.041 & +0.112 \\
C (+SOC暴露)    & 13 & 0.861$\pm$0.041 & $-$0.010 \\
D (+dQ/dV)      & 18 & 0.881$\pm$0.075 & +0.020 \\
\bottomrule
\end{tabularx}
\end{table}
...
策略参数（B，+0.112）是最大正向增量；dQ/dV统计量（D，+0.020）有温和增益。\textbf{SOC区间倍率暴露（C）无增益（$\Delta$R$^2$=$-$0.010）}——这与它们由C1/Q1/C2确定性推导、高度共线的判断一致，印证了``40--80\%危险区间''与``Q1重要''部分是同一条信息的两种表述。
...
\textbf{指标} & \textbf{数值} \\
\midrule
Test MAE  & 143$\pm$46 cycles \\
Test RMSE & 201$\pm$75 cycles \\
\textbf{Test R$^2$} & \textbf{0.726$\pm$0.177} \\
\bottomrule
\end{tabularx}
\end{table}
...
10次分层留出的R$^2$均值0.726，但标准差高达0.177——说明单次划分的R$^2$（如原报告的0.878）置信区间很宽，不足以支撑``显著优于''的结论。模型对未见电池有中等泛化能力，但受测试集划分影响较大。
...
IG归因最强的动态特征是内阻（IR）；SOC区间倍率（Crate\_*）与策略参数（C1/C2）在IG与置换重要性中排名靠前。注意：置换重要性高反映模型``依赖''该特征，并不等价于对泛化有益——消融已显示SOC区间特征无R$^2$增益（与C1/Q1/C2共线）。逐特征归因值见\texttt{results/metrics.json}。
...
训练模型的rollout模式与随机初始化基线明显不同，说明注意力学到了非平凡结构。\textbf{时间轴打乱对照}：打乱100个时间步后重训，测试R$^2$从0.741降至0.709（仅下降0.03），说明模型主要依赖退化``水平''信息（SOH/IR当前值）而非严格的时间顺序——因此不宜过度解读``Transformer捕捉长程时序依赖''的叙事。
...
\begin{enumerate}[label=(\arabic*), leftmargin=*]
    ...
    \item \textbf{SOC 40--80\% 区间降低倍率}：该区间为``危险区间''，高倍率充电导致不可逆损伤。Spearman分析表明该区间C-rate与寿命负相关性最强（$\rho=-0.330$）；
...
\begin{enumerate}[label=(\arabic*), leftmargin=*]
    ...
    \item \textbf{Transformer在公平对比下表现最优}。四层渐进式特征体系下，5折交叉验证R$^2$达0.827（Ridge公平对比0.577），留出测试集（10次分层划分）R$^2$为$0.726\pm0.177$；
